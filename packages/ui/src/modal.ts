...
class WalletPickerView {
    ...
    constructor(options: WalletUiOptions, presentation: WalletPickerPresentation) {
        ...
        this.recentWalletId = this.readRecentWalletId();
    ...
    private getWallets(): WalletMetadata[] {
        ...
        const orderedWallets = walletOrder?.length
            ...
            : wallets;
        return this.sortWalletsByRecent(orderedWallets);
    }
    private sortWalletsByRecent(wallets: WalletMetadata[]): WalletMetadata[] {
        const recentWalletId = this.recentWalletId;
        if (!recentWalletId)
            return wallets;
        const index = wallets.findIndex((wallet) => wallet.id === recentWalletId);
        if (index <= 0)
            return wallets;
        const next = wallets.slice();
        const [recent] = next.splice(index, 1);
        next.unshift(recent);
        return next;
    }
    private readRecentWalletId(): string | undefined {
        if (typeof window === "undefined")
            return undefined;
        try {
            return window.localStorage?.getItem(RECENT_WALLET_STORAGE_KEY) || undefined;
        }
        catch {
            return undefined;
        }
    }
    private writeRecentWalletId(adapterId: string) {
        this.recentWalletId = adapterId;
        if (typeof window === "undefined")
            return;
        try {
            window.localStorage?.setItem(RECENT_WALLET_STORAGE_KEY, adapterId);
        }
        catch {
            // Storage can be unavailable in private or embedded contexts; in-memory MRU still applies.
        }
    }
    ...
    private renderNetworkBadge() {
        ...
        if (!network || network.networkType === "MAINNET")
            return "";
        const { className, label } = this.resolveNetworkBadgeStyle(network);
        const dot = `<span class="xwk-network-dot" aria-hidden="true"></span>`;
        return `<div class="xwk-network-row"><span class="xwk-network-badge ${className}">${dot}${this.escapeHtml(label)}</span></div>`;
    }
    private resolveNetworkBadgeStyle(network: WalletNetwork): NetworkBadgeStyle {
        ...
        if (isEvm) {
            return {
                label: "XRPL EVM",
                className: "xwk-network-badge-evm"
            };
        }
        if (network.networkType === "DEVNET") {
            return {
                label: "DEVNET",
                className: "xwk-network-badge-devnet"
            };
        }
        if (network.networkType === "TESTNET") {
            return {
                label: "TESTNET",
                className: "xwk-network-badge-testnet"
            };
        }
        return {
            label: (network.name || "CUSTOM").toUpperCase().slice(0, 10),
            className: "xwk-network-badge-custom"
        };
    }
    ...
    private ensureStyles(theme: Required<WalletUiTheme>, layout: WalletUiLayout, size: WalletUiSize, textSize: WalletUiTextSize) {
        ...
        ensureWalletStyle("xwk-modal", styles, this.options.nonce);
    }
    ...
    private renderWalletBadges(wallet: WalletMetadata, layout: WalletUiLayout) {
        ...
        if (!badges.length && wallet.id === this.recentWalletId)
            badges.push(`<span class="xwk-wallet-badge xwk-recent">${this.escapeHtml(messages.recent || "Recent")}</span>`);
        if (!badges.length)
            return "";
