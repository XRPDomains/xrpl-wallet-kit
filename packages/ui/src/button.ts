...
import type { WalletAccountPanelMode, WalletBalance, WalletButtonIconConfig, WalletButtonOptions, WalletButtonTarget, WalletIdentity, WalletIdentityResolver, XrpDomainsResolverOptions } from "./types";
import type { WalletMetadata } from "@xrpl-wallet-kit/core";
export class WalletButtonController {
  private target?: HTMLElement;
  private options: WalletButtonOptions & Required<Pick<WalletButtonOptions, "label" | "icon" | "showAdapterIcon" | "showChevron" | "showWeb3Name" | "fallbackToAddress" | "copyAddress" | "showAddressQr" | "showRecentTransactions" | "maxVisibleTransactions" | "explorer" | "disconnect" | "accountPanel" | "accountPanelMode" | "showBalance" | "size" | "variant" | "themeMode" | "theme">>;
  ...
  private bindPanelActions(root: ParentNode): void {
    ...
    root.querySelector<HTMLButtonElement>("[data-xwk-tx-history]")?.addEventListener("click", (event) => {
      event.stopPropagation();
  ...
  private openTransactionHistory(): void {
    if (!this.options.manager.getSession()) return;
    this.panelOpen = true;
    this.addressQrOpen = false;
  ...
  private renderButton(): string {
    ...
    if (this.connecting && !session) {
      const icon = this.options.showAdapterIcon
        ? `<span class="xwk-button-icon xwk-button-icon-loading" aria-hidden="true"><span class="xwk-skeleton xwk-skeleton-circle xwk-skeleton-button-icon"></span></span>`
        : "";
      return `<button class="xwk-account-button xwk-account-button-loading" data-xwk-wallet-button type="button" aria-busy="true" aria-label="${this.escapeHtml(messages.connecting)}">${icon}<span class="xwk-button-label xwk-button-label-loading" aria-hidden="true"><span class="xwk-skeleton xwk-skeleton-line xwk-skeleton-button-label"></span></span></button>`;
    }
    ...
    const balance = session && this.options.showBalance && this.balanceLoading
      ? `<span class="xwk-button-balance xwk-button-balance-loading" aria-hidden="true"><span class="xwk-skeleton xwk-skeleton-button-balance"></span></span>`
    ...
    const icon = this.options.showAdapterIcon
      ? connected
        ? this.renderWalletIcon(session?.wallet)
        : this.renderPreConnectIcon()
      : "";
  ...
  private renderPanel(): string {
    ...
    const session = this.options.manager.getSession();
    if (!session) return "";
    const content = this.addressQrOpen
      ? this.renderAddressQrPanelContent(session)
      : this.historyOpen
      ? this.renderTransactionHistoryPanelContent(session)
      : this.renderPanelContent(session);
    ...
    const panelStateClass = `${this.addressQrOpen ? " xwk-account-panel-address-qr" : ""}${this.historyOpen ? " xwk-account-panel-history" : ""}${hasTransactions ? " xwk-account-panel-with-transactions" : ""}`;
    const leading = this.addressQrOpen || this.historyOpen
    ...
    const dropdownBack = this.addressQrOpen || this.historyOpen
      ? `<div class="xwk-account-dropdown-header"><button class="xwk-account-back" type="button" data-xwk-account-back aria-label="${this.escapeHtml(messages.back)}">${this.backIcon()}</button><strong>${this.escapeHtml(title)}</strong></div>`
      : "";
    return `<div class="xwk-account-panel xwk-account-panel-dropdown${panelStateClass}" role="dialog" aria-label="${this.escapeHtml(title)}">${dropdownBack}${content}</div>`;
  }
  ...
  private renderPanelContent(session: WalletSession): string {
    ...
    const balance = this.options.showBalance && this.balanceLoading
      ? `<span class="xwk-pill-balance xwk-account-balance-loading" role="status" aria-label="${this.escapeHtml(messages.connecting)}"><span class="xwk-skeleton xwk-skeleton-balance"></span></span>`
      : this.options.showBalance && this.activationStatus !== "unfunded" && this.balance?.formatted
      ? `<span class="xwk-pill-balance">${this.escapeHtml(this.balance.formatted)}</span>`
      : "";
    const addressQrButton = this.renderAddressQrButton(address);
    const identityResolving = this.isIdentityResolving(session);
    const hasIdentity = this.shouldShowWeb3Name() && this.identityName;
    const addressLabel = address
      ? `<span class="xwk-pill-address">${this.escapeHtml(this.formatAddress(address))}</span>`
      : "";
    const divider = addressLabel && balance ? `<span class="xwk-pill-divider" aria-hidden="true"></span>` : "";
    const metaPill = addressLabel || balance || addressQrButton
      ? `<div class="xwk-account-meta-pill">${addressLabel}${divider}${balance}${addressQrButton}</div>`
      : "";
    const actions = `<div class="xwk-account-panel-actions">${copy}${explorer}${disconnect}</div>`;
    const accountName = identityResolving
      ? `<div class="xwk-account-name xwk-account-name-skeleton" role="status" aria-label="${this.escapeHtml(messages.connecting)}"><span class="xwk-skeleton xwk-skeleton-name"></span></div>`
      : hasIdentity
      ? `<div class="xwk-account-name">${this.escapeHtml(label)}</div>`
      : "";
    const transactions = this.renderTransactionHistory(session);
    return `<div class="xwk-account-hero">${this.renderAccountAvatar(session, identityResolving)}</div>${accountName}${metaPill}${notActivated}${transactions}${actions}`;
  }
  ...
  private renderTransactionHistory(session: WalletSession): string {
    ...
    const transactions = allTransactions.slice(0, Math.min(3, allTransactions.length));
    const messages = this.messages();
    const count = allTransactions.length > 9 ? "9+" : String(allTransactions.length);
    const historyAction = allTransactions.length > transactions.length
      ? `<button class="xwk-tx-header-link" type="button" data-xwk-tx-history>${this.escapeHtml(messages.view)}${this.chevronRightIcon()}</button>`
      : "";
    const list = `<div class="xwk-tx-list" role="list"><div class="xwk-tx-list-inner">${transactions.map((transaction) => this.renderTransactionRow(transaction)).join("")}</div></div>`;
    return `<section class="xwk-tx-section" aria-label="${this.escapeHtml(messages.recentTransactions)}"><div class="xwk-tx-header"><span>${this.escapeHtml(messages.recentTransactions)}</span><span class="xwk-tx-count">${this.escapeHtml(count)}</span>${historyAction}</div>${list}</section>`;
  }

  private renderTransactionHistoryPanelContent(session: WalletSession): string {
    const messages = this.messages();
    const transactions = this.getVisibleTransactions(session, Math.max(10, this.options.maxVisibleTransactions));
    if (!transactions.length) return `<div class="xwk-tx-empty">${this.escapeHtml(messages.noRecentTransactions)}</div>`;
    return `<section class="xwk-tx-history-view" aria-label="${this.escapeHtml(messages.recentTransactions)}"><div class="xwk-tx-list" role="list"><div class="xwk-tx-list-inner">${transactions.map((transaction) => this.renderTransactionRow(transaction)).join("")}</div></div></section>`;
  }

  private renderTransactionRow(transaction: WalletTransaction): string {
    const messages = this.messages();
    const label = this.transactionStatusLabel(transaction);
    const title = this.transactionTitle(transaction);
  ...
  private getVisibleTransactions(session: WalletSession, limit = this.options.maxVisibleTransactions): WalletTransaction[] {
    const address = this.getSessionAddress(session);
  ...
  private renderAccountAvatar(session: WalletSession, identityResolving = false): string {
    ...
    return `<div class="xwk-account-art xwk-account-art-tone-${this.getAddressGradientTone(this.getSessionAddress(session))}"><span></span></div>`;
  }
  ...
  private isIdentityResolving(session: WalletSession | null | undefined): boolean {
  ...
  private renderPreConnectIcon(): string {
  ...
  private renderCustomHtmlIcon(icon: Extract<WalletButtonIconConfig, { type: "html" }>): string {
    const aria = icon.ariaLabel ? ` role="img" aria-label="${this.escapeHtml(icon.ariaLabel)}"` : ` aria-hidden="true"`;
    return `<span class="xwk-button-icon xwk-button-icon-custom xwk-button-icon-custom-html"${aria}>${icon.html}</span>`;
  }
  ...
  private chevronRightIcon(): string {
    return `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="m9 6 6 6-6 6" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
  }
  ...
  private ensureStyles(): void {
    ...
    ensureWalletStyle("xwk-button", styles, this.options.nonce);
  }
  ...
  private renderStyles(): string {
        ...
        const avatarToneStyles = `.xwk-account-art-tone-0{--xwk-avatar-from:#0ea5e9;--xwk-avatar-to:#6366f1;--xwk-avatar-spot:#93c5fd}.xwk-account-art-tone-1{--xwk-avatar-from:#14b8a6;--xwk-avatar-to:#2563eb;--xwk-avatar-spot:#99f6e4}.xwk-account-art-tone-2{--xwk-avatar-from:#f97316;--xwk-avatar-to:#db2777;--xwk-avatar-spot:#fed7aa}.xwk-account-art-tone-3{--xwk-avatar-from:#8b5cf6;--xwk-avatar-to:#06b6d4;--xwk-avatar-spot:#ddd6fe}.xwk-account-art-tone-4{--xwk-avatar-from:#22c55e;--xwk-avatar-to:#0f766e;--xwk-avatar-spot:#bbf7d0}.xwk-account-art-tone-5{--xwk-avatar-from:#f43f5e;--xwk-avatar-to:#7c3aed;--xwk-avatar-spot:#fecdd3}`;
        ...
        return `${baseStyles}${avatarToneStyles}${this.renderSkeletonPrimitiveStyles(theme, iconBorder)}`;
  }

  private renderSkeletonPrimitiveStyles(theme: ResolvedTheme, iconBorder: string): string {
    return `.xwk-skeleton-circle{border-radius:999px}.xwk-skeleton-line{height:14px;border-radius:4px}.xwk-skeleton-pill{border-radius:999px}.xwk-skeleton-card,.xwk-skeleton-button{border-radius:${theme.walletRadius}}.xwk-skeleton-row{align-items:center;display:flex;gap:12px;width:100%}.xwk-skeleton-button-icon{height:28px;width:28px}.xwk-skeleton-button-label{height:14px;width:92px}.xwk-account-button-loading .xwk-button-label{align-items:center;display:inline-flex}.xwk-button-icon-loading{background:transparent;border-color:${iconBorder};padding:0}.xwk-account-avatar-skeleton{background:${theme.surface};border:1px solid ${iconBorder};box-sizing:border-box;overflow:hidden}.xwk-account-avatar-skeleton>span{height:100%;width:100%}.xwk-tx-skeleton-row{grid-template-columns:24px minmax(0,1fr) auto}.xwk-tx-skeleton-main{display:grid;gap:6px;min-width:0}.xwk-tx-skeleton-label{height:13px;width:86px}.xwk-tx-skeleton-hash{height:11px;width:132px}@media(prefers-reduced-motion:reduce){.xwk-skeleton{animation:none!important;background:${theme.surfaceHover}!important}}`;
  }
  ...
  private getAddressGradientTone(address: string): number {
    let hash = 0;
...
function compactButtonOptions<T extends WalletButtonOptions>(options: T): T {
