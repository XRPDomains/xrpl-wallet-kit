...
export class WalletButtonController {
  ...
  private renderPanel(): string {
    ...
    const dropdownBack = this.addressQrOpen
  ...
  private renderPanelContent(session: WalletSession): string {
    ...
    const balance = this.options.showBalance && this.balanceLoading
      ? `<div class="xwk-account-balance xwk-account-balance-loading" role="status" aria-label="${this.escapeHtml(messages.connecting)}"><span class="xwk-skeleton xwk-skeleton-balance"></span></div>`
    ...
    const addressLabel = (hasIdentity || identityResolving) && address
    ...
    const accountName = identityResolving
      ...
      : !hasIdentity && addressQrButton && this.options.fallbackToAddress
  ...
  private renderTransactionHistory(session: WalletSession): string {
    ...
    const listClass = this.txSectionOpen ? "open" : "closed";
    const list = `<div class="xwk-tx-list ${listClass}" role="list" aria-hidden="${this.txSectionOpen ? "false" : "true"}"><div class="xwk-tx-list-inner">${transactions.map((transaction) => this.renderTransactionRow(transaction)).join("")}</div></div>`;
