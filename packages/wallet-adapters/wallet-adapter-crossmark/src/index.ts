...
export class CrossmarkAdapter extends BaseWalletAdapter {
  ...
  isAvailable(): boolean { return Boolean(this.getProvider(false)?.sync?.isInstalled?.()); }

  async connect(options: ConnectOptions) {
    const result = await this.getProvider().methods.signInAndWait(this.randomHex());
  ...
  async signMessage(request: SignMessageRequest) {
    const result = await this.getProvider().methods.signInAndWait(request.message);
    return { signature: result.response?.data?.signature, raw: result };
  }

  async signAndSubmit(request: SignAndSubmitRequest) {
    return normalizeTxResult(await this.getProvider().methods.signAndSubmitAndWait(request.txJson));
  }
