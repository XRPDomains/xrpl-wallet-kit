...
import { BaseWalletAdapter, createBrowserWalletStorage, createWalletError, normalizeTxResult, utf8ToHex } from "@xrpl-wallet-kit/core";
...
export interface XamanSdkLike {
  ...
  environment?: {
    ...
    retrieved?: Promise<unknown>;
    success?: Promise<unknown>;
    openid?: Promise<XamanAuthResult["me"] | undefined>;
  };
...
export class XamanAdapter extends BaseWalletAdapter {
  ...
  async signMessage(request: SignMessageRequest) {
    ...
    if (!result?.meta?.signed) throw createWalletError.signRejected(result);
  ...
  private async waitForSdkReady(sdk: XamanSdkLike): Promise<void> {
    const readySignals = [
      sdk.environment?.ready,
      sdk.environment?.retrieved,
      sdk.environment?.success
    ].filter((signal): signal is Promise<unknown> => Boolean(signal));

    if (!readySignals.length) return;

    await Promise.race([
      Promise.race(readySignals).then(() => undefined),
      new Promise<void>((resolve) => setTimeout(resolve, XAMAN_RESTORE_READY_TIMEOUT_MS))
    ]);
  }
