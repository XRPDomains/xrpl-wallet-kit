...
import type { ConnectOptions, ConnectResult, SignAndSubmitRequest, SignMessageRequest, WalletCapabilities, WalletMetadata, WalletSession } from "@xrpl-wallet-kit/core";
...
export class XamanAdapter extends BaseWalletAdapter {
  ...
  async recoverSession(options: ConnectOptions): Promise<ConnectResult | null> {
    if (!this.hasPendingRecoveryMarker()) return null;
    try {
      const result = await this.authorize();
