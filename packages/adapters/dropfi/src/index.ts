import { BaseWalletAdapter, createWalletError, normalizeTxResult, pickPath } from "@xrpl-wallet-kit/core";
import type { ConnectOptions, SignAndSubmitRequest, SignMessageRequest, WalletCapabilities, WalletMetadata, WalletNetwork, WalletSession } from "@xrpl-wallet-kit/core";
...
export interface DropFiProvider {
  ...
  connect?: (data?: unknown) => Promise<boolean | string | DropFiState | { address?: string; selectedAddress?: string; connectedAccounts?: string[]; accounts?: string[]; publicKey?: string }>;
...
export class DropFiAdapter extends BaseWalletAdapter {
  ...
  async restoreSession(session: WalletSession) {
    ...
    const publicKey = await this.resolvePublicKey(provider, undefined, state);

    this.activeAddress = address;
    return {
      account: { ...session.account, address, publicKey: session.account.publicKey ?? publicKey },
      session: { ...session, account: { ...session.account, address, publicKey: session.account.publicKey ?? publicKey } },
      raw: { address, publicKey, connected }
    };
  }
  ...
  async switchNetwork(network: WalletNetwork) {
    const provider = this.provider();
    if (!provider.switchNetwork) throw createWalletError.unsupportedMethod("switchNetwork", this.metadata.name);
    const raw = await provider.switchNetwork(String(network.networkId ?? network.id));
    return { network, raw };
  }
  ...
  private async resolveAddress(
    provider: DropFiProvider,
    connected: boolean | string | DropFiState | { address?: string; selectedAddress?: string; connectedAccounts?: string[]; accounts?: string[]; publicKey?: string },
