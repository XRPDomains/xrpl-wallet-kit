...
## 12. Open Questions (cần confirm trước khi code)

**Q1: Auto-capture từ `signAndSubmit`?**
Hiện tại `manager.addTransaction()` đã được gọi tự động trong `manager.ts` khi `signAndSubmit` thành công. Chỉ cần bật persistence là đủ, KHÔNG cần thêm auto-capture logic.
→ **Xác nhận:** Đúng, xem `manager.ts:375` — `this.addTransaction({ hash, status: "submitted" })` được gọi sau `signAndSubmit`.

**Q2: `description` và `amount` lấy từ đâu?**
Kit không parse `txJson` để extract amount/destination (vì adapter-agnostic). App phải set qua `metadata: { amount, currency, destination }` khi gọi `addTransaction`. UI sẽ read từ `tx.metadata` nếu có.
→ Document rõ pattern này trong docs.

**Q3: Khi app reload, tx `"submitted"` còn trong store có nên tiếp tục poll confirmation?**
Đề xuất: **Có, nếu `transactionConfirmation.enabled !== false`** và tx `submittedAt > Date.now() - timeoutMs`. Phức tạp hơn một chút nhưng UX đúng hơn.

**Q4: Có nên thêm tab "Transactions" riêng trong panel hay để inline?**
Đề xuất: **Inline section** (không có tab) — đơn giản hơn, không cần state management tab. Nếu danh sách dài thì scroll. Có thể thêm "View all" link mở XRPL Explorer account page.

**Q5: WalletInline cũng có transaction section không?**
Không trong Phase 1-4. `WalletInline` là embed không phải panel → layout khác. Để future sprint.
