...
2. Update internal `@xrpl-wallet-kit/*` dependency versions, the browser banner, website navigation version, and legacy example cache query. Website widgets load the latest browser package from jsDelivr and do not keep a local bundle copy.
3. Run `npm run build:browser`, then run `npm run smoke:release` and `npm --prefix website run build`.
